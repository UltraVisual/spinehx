...
#if display
import spinehx.AnimationStateTest;
...
class ApplicationMain
{
	...
	public static function main()
	{
		...
		wx.App.boot(function()
		{
			...
			frame = wx.Frame.create(null, null, "spinehx - AnimationStateTest", null, { width: 320, height: 480 });
			
			#if openfl
			var stage = wx.NMEStage.create(frame, null, null, { width: 320, height: 480 });
			#end
			
			spinehx.AnimationStateTest.main();
		...
		flash.Lib.create(function()
			{ 
				...
				for (methodName in Type.getClassFields(spinehx.AnimationStateTest))
				{
					if (methodName == "main")
					{
						hasMain = true;
				...
				if (hasMain)
				{
					Reflect.callMethod (spinehx.AnimationStateTest, Reflect.field (spinehx.AnimationStateTest, "main"), []);
				}
				else
				{
...
class DocumentClass extends spinehx.AnimationStateTest {}
