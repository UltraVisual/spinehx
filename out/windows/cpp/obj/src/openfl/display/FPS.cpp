...
int inCol = __o_inCol.Default(0);
{
	...
	HX_STACK_LINE(24)
	this->set_defaultTextFormat(::flash::text::TextFormat_obj::__new(HX_CSTRING("_sans"),(int)12,inCol,null(),null(),null(),null(),null(),null(),null(),null(),null(),null()));
	HX_STACK_LINE(26)
	this->set_text(HX_CSTRING("FPS: "));
	HX_STACK_LINE(28)
	this->times = Array_obj< Float >::__new();
	HX_STACK_LINE(29)
	this->addEventListener(::flash::events::Event_obj::ENTER_FRAME,this->onEnter_dyn(),null(),null(),null());
}
;
	return null();
}
...
{
		HX_STACK_PUSH("FPS::onEnter","openfl/display/FPS.hx",39);
		HX_STACK_THIS(this);
		HX_STACK_ARG(_,"_");
		HX_STACK_LINE(40)
